...
public class DebugxConsole : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        EditorGUILayout.LabelField("Playing Settings", EditorStyle.Get.TitleStyle_2);
        if (!Application.isPlaying) EditorGUILayout.HelpBox("在游戏运行时才能设置这部分内容", MessageType.Info);
        EditorGUI.BeginDisabledGroup(!Application.isPlaying);
        EditorGUILayout.LabelField("Toggle", EditorStyle.Get.TitleStyle_3);
...
#if UNITY_EDITOR
        EditorGUILayout.Space();
        EditorGUILayout.LabelField("Test", EditorStyle.Get.TitleStyle_2);
